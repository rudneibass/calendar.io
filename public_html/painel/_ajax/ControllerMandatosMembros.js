function insertTab2Form1() {
    $.ajax({
        url: '../../_php/Dispatch.php?controller=ControllerMandatosMembros&&action=insert',
        type: 'POST',
...
function locateTab2Form1(id) {
    ...
    $.ajax({
        url: '../../_php/Dispatch.php?controller=ControllerMandatosMembros&&action=locate',
        cache: false,
        type: 'POST',
        data: {id: id},
        dataType: "json",
                ...
                for (var i = pagina * tamanhoPagina; i < retorno.length && i < (pagina + 1) * tamanhoPagina; i++) {
                    ...
                    if (retorno[i].ativo != 'off') {
                        $("#tab-2-tbody-1").append(
                            ...
                            .append($('<td style="background-color: #ededed">').append('<center><button class="btn btn-warning btn-sm" onclick="populaFormTab2Form1(' + retorno[i].id + ')" ><i class="fa fa-pencil"></i></button></center>'))
                            );
                    }
                }
            }
...
function populaFormTab2Form1(id) {
    $.ajax({
        url: '../../_php/Dispatch.php?controller=ControllerMandatosMembros&&action=locate&&id=' + id,
        type: 'POST',
        dataType: "json",
        data: {id: id},
...
function  updateTab2Form1(id) {
    $.ajax({
        url: '../../_php/Dispatch.php?controller=ControllerMandatosMembros&&action=update&&id=' + id,
        type: 'POST',
        data: $('#tab-2-form-1').serialize(),
        beforeSend: function () {
