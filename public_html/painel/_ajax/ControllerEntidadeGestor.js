function insertTab2Form1() {
    $.ajax({
        url: '../../_php/Dispatch.php?controller=ControllerEntidadeGestor&&action=insert',
        type: 'POST',
...
function locateTab2Form1(id) {
    ...
    $.ajax({
        url: '../../_php/Dispatch.php?controller=ControllerEntidadeGestor&&action=locate',
        cache: false,
        type: 'POST',
        data: {id: id},
        dataType: "json",
                ...
                for (var i = pagina * tamanhoPagina; i < retorno.length && i < (pagina + 1) * tamanhoPagina; i++) {
                    ...
                    if (retorno[i].ativo === 'on') {
                        ...
                        $("#tab-2-tbody-1").append(
                                ...
                                .append($('<td style="background-color: #ededed">').append('<center><span class="badge badge-warning" onclick="populaFormTab2Form1(' + retorno[i].id + ')" ><i class="fa fa-pencil"></i></span></center>'))
...
function populaFormTab2Form1(id) {
    $.ajax({
        url: '../../_php/Dispatch.php?controller=ControllerEntidadeGestor&&action=locate&&id=' + id,
        type: 'POST',
        dataType: "json",
        data: {id: id},
...
function  deleteTab2Form1(id) {
    if(confirm("TEM CERTEZA QUE DESEJA EXCLUIR ESSE REGISTRO?")){$.ajax({
        url: '../../_php/Dispatch.php?controller=ControllerEntidadeGestor&&action=delete',
        type: 'POST',
        data: {id: id},
        beforeSend: function () {
...
function  updateTab2Form1(id) {
    $.ajax({
        url: '../../_php/Dispatch.php?controller=ControllerEntidadeGestor&&action=update&&id=' + id,
        type: 'POST',
        data: $('#tab-2-form-1').serialize(),
        beforeSend: function () {
            toastr["info"]("Aguarde, estamos providenciando o envio de suas informações!")
