...
function populaForm(id) {
    $.ajax({
        url: '../../_php/Dispatch.php?controller=ControllerDiarias&&action=locate',
        type: 'POST',
        dataType: "json",
        data: {id: id},
        beforeSend: function () {
...
//                    var tbody = $('table > tbody');
                    ...
                    for (var i = pagina * tamanhoPagina; i < retorno.length && i < (pagina + 1) * tamanhoPagina; i++) {
                        ...
                        $("#tbody").append(
                                ...
                                .append($('<td>').append('<b>Exerc.: </b>' + retorno[i].exercicio + '<br/> <b>Data: </b>' + retorno[i].data_diaria + '<br/> <b>Num.: </b>' + retorno[i].numero + '<br/><b>Benef.: </b>' + retorno[i].beneficiario))
                                .append($('<td>').append('<b>Qtde: </b>' + retorno[i].quantidade + '<br/><b>Val. Unit.: </b> R$' + retorno[i].valor_unitario + '<br/><b>Val. Total: </b> R$' + retorno[i].valor_total))
                                .append($('<td width="5%">').append('<center><button class="btn btn-secondary btn-sm"      onclick="listarArquivos(' + retorno[i].id + ', \'diarias\',\'tbody_arquivos_modal\')" data-toggle="modal" data-target="#modal-files"><i class="fa fa-file-text"></i></button></center>'))                                
...
function  update(id) {
    $.ajax({
        url: '../../_php/Dispatch.php?controller=ControllerDiarias&&action=update&&id=' + id,
        type: 'POST',
...
function insert() {
    $.ajax({
        url: '../../_php/Dispatch.php?controller=ControllerDiarias&&action=insert',
        type: 'POST',
        data: $('#tab-1-form-1').serialize(),
        beforeSend: function () {
        ...
        success: function (data) {
            ...
            if (!isNaN(data)) {
                ...
                document.getElementById("tab-2-btn-show-modal-upload").addEventListener('click', () => {
                    showModalUpload('modal-upload', 'ControllerUploadMultiplo', 'analisaArquivo', data , 'diarias')  
                });
...
function deletar(id) {
    if (confirm("TEM CERTEZA QUE DESEJA EXCLUIR ESSE REGISTRO?")) {
        $.ajax({
            url: '../../_php/Dispatch.php?controller=ControllerDiarias&&action=delete',
            type: "POST",
            data: {id: id},
            success: function (data) {
                $("#linha" + id).remove();
...
function importarJson() {
    ...
    $.ajax({
      url: '../../_php/Dispatch.php?controller=ControllerDiarias&&action=importarJson',
      type: 'POST',
      data: formData,
      contentType: false,
      processData: false,
