...
function populaForm(id) {
    $.ajax({
        url: '../../_php/Dispatch.php?controller=ControllerOpcoesTabelasColunas&&action=locate',
        type: 'POST',
        dataType: "json",
        data: {id: id},
        beforeSend: function () {
        ...
        success: function (json) {
            ...
            $('#optGroupcolunas').append('<option value='+data.coluna+'>'+data.coluna)
...
function locate() {
    ...
    $('#thead').append(
            ...
            .append($('<th>').append('Tabela'))
			.append($('<th>').append('Coluna'))
            .append($('<th>').append('Nome'))
            .append($('<th>').append('Valor'))
            .append($('<th>').append('optGroup'))
            ...
            if (!retorno[0]) {
                ...
                function paginar() {
                    ...
                    for (var i = pagina * tamanhoPagina; i < retorno.length && i < (pagina + 1) * tamanhoPagina; i++) {
                        ...
                        if(retorno[i].ativo == 'S'){
                            $("#tbody").append(
                                $('<tr id="linha' + retorno[i].id + '">')
                                .append($('<td style="width: 5%">').append(retorno[i].id))
								.append($('<td>').append(retorno[i].tabela))
                                .append($('<td>').append(retorno[i].coluna))
                                .append($('<td>').append(retorno[i].nome))
                                .append($('<td>').append(retorno[i].valor))
                                .append($('<td>').append(retorno[i].opcoes_grupo))
                                .append($('<td>').append(retorno[i].ativo))
                        ...
                        if(retorno[i].ativo == 'N'){
                            $("#tbody").append(
                                $('<tr id="linha' + retorno[i].id + '">')
                                .append($('<td style="background-color: #ededed; width: 5%">').append(retorno[i].id))
								.append($('<td style="background-color: #ededed">').append(retorno[i].tabela))
                                .append($('<td style="background-color: #ededed">').append(retorno[i].coluna))
                                .append($('<td style="background-color: #ededed">').append(retorno[i].nome))
                                .append($('<td style="background-color: #ededed">').append(retorno[i].valor))
                                .append($('<td style="background-color: #ededed">').append(retorno[i].opcoes_grupo))
                                .append($('<td style="background-color: #ededed">').append(retorno[i].ativo))
                                .append($('<td style="background-color: #ededed; width: 5%">').append('<center><button class="btn btn-secondary btn-sm" onclick="vizualizar(' + retorno[i].id + ')" ><i class="fa fa-pencil"></i></button></center>'))
                                .append($('<td style="background-color: #ededed; width: 5%">').append('<center><button class="btn btn-secondary btn-sm" onclick="deletar(' + retorno[i].id + ')" ><i class="fa fa-remove"></i></button></center>'))
                            )
                        }
...
function  update(id) {
    $.ajax({
        url: '../../_php/Dispatch.php?controller=ControllerOpcoesTabelasColunas&&action=update&&id=' + id,
        type: 'POST',
...
function insert() {
    $.ajax({
        url: '../../_php/Dispatch.php?controller=ControllerOpcoesTabelasColunas&&action=insert',
        type: 'POST',
        data: $('#tab-1-form-1').serialize(),
        beforeSend: function () {
...
function deletar(id) {
    if (confirm("TEM CERTEZA QUE DESEJA EXCLUIR ESSE REGISTRO?")) {
        $.ajax({
            url: '../../_php/Dispatch.php?controller=ControllerOpcoesTabelasColunas&&action=delete',
            type: "POST",
            data: {id: id},
            success: function (data) {
                $("#linha" + id).remove();
