...
function populaForm(id) {
    $.ajax({
        url: '../../_php/Dispatch.php?controller=ControllerTransparenciaCovid19&&action=locate',
        type: 'POST',
        dataType: "json",
        data: {id: id},
        success: function (json) {
            ...
            $("#btn-upload-update").attr("onclick", "btnUploadUpdate(" + data.id + ", \'" + data.tbl + "\')");
...
function locate() {
    ...
    $('thead').append(
            ...
            .append($('<th>').append('Receitas'))
            .append($('<th>').append('Empenhos'))
            .append($('<th>').append('Liquidacões'))
            .append($('<th>').append('Pagamentos'))
            .append($('<th>').append(''))
            .append($('<th>').append(''))
            );
...
//                    var tbody = $('table > tbody');
                    ...
                    for (var i = pagina * tamanhoPagina; i < retorno.length && i < (pagina + 1) * tamanhoPagina; i++) {
                        ...
                        $("#tbody").append(
                                ...
                                .append($('<td>').append(retorno[i].receitas))
                                .append($('<td>').append(retorno[i].empenhos))
                                .append($('<td>').append(retorno[i].liquidacoes))
                                .append($('<td>').append(retorno[i].pagamentos))                        
                    ...
                    $('#proximo').click(function () {
                        if (pagina < retorno.length / tamanhoPagina - 1) {
                            ...
                            $('thead').append(
                                    ...
                                    .append($('<th>').append('Suspeitos'))
                                    .append($('<th>').append('Confirmados'))
                                    .append($('<th>').append('Descartados'))
                                    .append($('<th>').append('Internados'))
                                    .append($('<th>').append('Óbitos'))
                                    .append($('<th>').append('Curados'))
                                    .append($('<th>').append('Notificados'))
                                    .append($('<th>').append(''))
                                    .append($('<th>').append(''))
                                    );
                            pagina++;
...
function  update(id) {
    $.ajax({
        url: '../../_php/Dispatch.php?controller=ControllerTransparenciaCovid19&&action=update&&id=' + id,
        type: 'POST',
...
function insert() {
    $.ajax({
        url: '../../_php/Dispatch.php?controller=ControllerTransparenciaCovid19&&action=insert',
        type: 'POST',
        data: $('#form_insert').serialize(),
        beforeSend: function () {
...
function deletar(id) {
    if (confirm("TEM CERTEZA QUE DESEJA EXCLUIR ESSE REGISTRO?")) {
        $.ajax({
            url: '../../_php/Dispatch.php?controller=ControllerTransparenciaCovid19&&action=delete',
            type: "POST",
            data: {id: id},
            success: function (data) {
                $("#linha" + id).remove();
