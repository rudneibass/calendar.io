...
function populaForm(id) {
    $.ajax({
        url: '../../_php/Dispatch.php?controller=ControllerLeis&&action=locate',
        type: 'POST',
        dataType: "json",
        data: {id: id},
        beforeSend: function () {
                ...
                function paginar() {
                    ...
                    for (var i = pagina * tamanhoPagina; i < retorno.length && i < (pagina + 1) * tamanhoPagina; i++) {
                        ...
                        $("#tbody").append(
                                ...
                                .append($('<td>').append('<b>Tipo: </b>' + retorno[i].tipo + '<br/> <b>Data: </b>' + retorno[i].data_lei + '<br/> <b>Publicação </b>' + retorno[i].data_publicacao + '<br/><b>Descrição: </b>' + retorno[i].descricao))
                                .append($('<td width="5%">').append('<center><button class="btn btn-secondary btn-sm"      onclick="listarArquivos(' + retorno[i].id + ', \'leis\',\'tbody_arquivos_modal\')"       data-toggle="modal" data-target="#modal-files"><i class="fa fa-file-text"></i></button></center>'))
...
function  update(id) {
    $.ajax({
        url: '../../_php/Dispatch.php?controller=ControllerLeis&&action=update&&id=' + id,
        type: 'POST',
...
function insert() {
    $.ajax({
        url: '../../_php/Dispatch.php?controller=ControllerLeis&&action=insert',
        type: 'POST',
        data: $('#tab-1-form-1').serialize(),
        beforeSend: function () {
            ...
            if (!isNaN(data)) {
               //toastr["success"]("Cadastro realizado com sucesso!");
                ...
                document.getElementById("tab-2-btn-show-modal-upload").addEventListener('click', () =>{
                    showModalUpload('modal-upload', 'ControllerUploadMultiplo', 'analisaArquivo', data , 'leis')  
                });

                bootbox.confirm({
                    title: "Atenção",
                    message: "Cadastro realizado com sucesso! Essa lei altera alguma outra?",
                    buttons: {
                        cancel: {
                            label: "Não",
...
function deletar(id) {
    if (confirm("TEM CERTEZA QUE DESEJA EXCLUIR ESSE REGISTRO?")) {
        $.ajax({
            url: '../../_php/Dispatch.php?controller=ControllerLeis&&action=delete',
            type: "POST",
            data: {id: id},
            success: function (data) {
                $("#linha" + id).remove();
...
function importarJson() {
    ...
    $.ajax({
      url: '../../_php/Dispatch.php?controller=ControllerLeis&&action=importarJson',
      type: 'POST',
      data: formData,
      contentType: false,
      processData: false,
