function insertTab2Form1() {
    $.ajax({
        url: '../../_php/Dispatch.php?controller=ControllerMateriasAutores&&action=insert',
        type: 'POST',
...
function locateTab2Form1(id) {
    $("#tab-2-tbody-1").empty();
    $.ajax({
        url: '../../_php/Dispatch.php?controller=ControllerMateriasAutores&&action=locate',
        cache: false,
        type: 'POST',
        data: {id: id},
        dataType: "json",
        ...
        success: function (retorno) {
            if (!retorno[0]) {
                $("#tab-2-echos-1").html("<h6 class='text-muted text-center'>Não há autores cadastrados!</h6>");
            } else {
                ...
                for (var i = pagina * tamanhoPagina; i < retorno.length && i < (pagina + 1) * tamanhoPagina; i++) {
                    $("#tab-2-tbody-1").append(
                            ...
                            .append($('<td style="width: 5%">').append(retorno[i].autor_subescritor))
...
function populaFormTab2Form1(id) {
    $.ajax({
        url: '../../_php/Dispatch.php?controller=ControllerMateriasAutores&&action=locate&&id=' + id,
        type: 'POST',
        dataType: "json",
        data: {id: id},
        beforeSend: function () {
...
function  updateTab2Form1(id) {
    $.ajax({
        url: '../../_php/Dispatch.php?controller=ControllerMateriasAutores&&action=update&&id=' + id,
        type: 'POST',
        data: $('#tab-2-form-1').serialize(),
        beforeSend: function () {
...
function deletarTab2Form1(id) {
    if (confirm("\u{26A0}\u{FE0F} TEM CERTEZA QUE DESEJA EXCLUIR ESSE REGISTRO?")) {
        $.ajax({
            url: '../../_php/Dispatch.php?controller=ControllerMateriasAutores&&action=delete',
            type: "POST",
            data: {id: id},
            success: function (data) {
                $("#linha" + id).remove();
