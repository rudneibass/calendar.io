...
function populaForm(id) {
    $.ajax({
        url: '../../_php/Dispatch.php?controller=ControllerPerguntas&&action=locate',
        type: 'POST',
        dataType: "json",
        data: {id: id},
...
function locate() {
    ...
    $('#thead').append(
            ...
            .append($('<th>').append('Pergunta'))
            .append($('<th>').append('Resposta'))
            .append($('<th>').append(''))
            .append($('<th>').append(''))
            );
...
//                    var tbody = $('table > tbody');
                    ...
                    for (var i = pagina * tamanhoPagina; i < retorno.length && i < (pagina + 1) * tamanhoPagina; i++) {
                        ...
                        $("#tbody").append(
                                ...
                                .append($('<td>').append(retorno[i].pergunta))
                                .append($('<td>').append(retorno[i].resposta))
...
function  update(id) {
    $.ajax({
        url: '../../_php/Dispatch.php?controller=ControllerPerguntas&&action=update&&id=' + id,
        type: 'POST',
...
function insert() {
    $.ajax({
        url: '../../_php/Dispatch.php?controller=ControllerPerguntas&&action=insert',
        type: 'POST',
        data: $('#form_insert').serialize(),
        beforeSend: function () {
...
function deletar(id) {
    if (confirm("TEM CERTEZA QUE DESEJA EXCLUIR ESSE REGISTRO?")) {
        $.ajax({
            url: '../../_php/Dispatch.php?controller=ControllerPerguntas&&action=delete',
            type: "POST",
            data: {id: id},
            success: function (data) {
                $("#linha" + id).remove();
