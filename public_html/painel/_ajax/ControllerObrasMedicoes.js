function insertTab2Form1() {
    $.ajax({
        url: '../../_php/Dispatch.php?controller=ControllerObrasMedicoes&&action=insert',
        type: 'POST',
...
function locateTab2Form1(id) {
    $("#tab-2-tbody-1").empty();
    $.ajax({
        url: '../../_php/Dispatch.php?controller=ControllerObrasMedicoes&&action=locate',
        cache: false,
        type: 'POST',
        data: {id: id},
        dataType: "json",
            ...
            if (!retorno[0]) {
                ...
                for (var i = pagina * tamanhoPagina; i < retorno.length && i < (pagina + 1) * tamanhoPagina; i++) {
                    $("#tab-2-tbody-1").append(
                            ...
                            .append($('<td>').append(retorno[i].responsavel))
                            .append($('<td>').append(retorno[i].data_medicao))
...
function populaFormTab2Form1(id) {
    $.ajax({
        url: '../../_php/Dispatch.php?controller=ControllerObrasMedicoes&&action=locate&&id=' + id,
        type: 'POST',
        dataType: "json",
        data: {id: id},
...
function  updateTab2Form1(id) {
    $.ajax({
        url: '../../_php/Dispatch.php?controller=ControllerObrasMedicoes&&action=update&&id=' + id,
        type: 'POST',
        data: $('#tab-2-form-1').serialize(),
        beforeSend: function () {
