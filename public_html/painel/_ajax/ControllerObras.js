...
function populaForm(id) {
    $.ajax({
        url: '../../_php/Dispatch.php?controller=ControllerObras&&action=locate',
        type: 'POST',
        dataType: "json",
        data: {id: id},
        beforeSend: function () {
                ...
                function paginar() {
                    ...
                    for (var i = pagina * tamanhoPagina; i < retorno.length && i < (pagina + 1) * tamanhoPagina; i++) {
                        ...
                        $("#tbody").append(
                                ...
                                .append($('<td>').append('<b> Exerc.: </b>' + retorno[i].exercicio + ' - <b>Data Inicio: </b>' + retorno[i].data_inicio + '<br/> <b>Local: </b>' + retorno[i].local))
                                .append($('<td style="width:60%">').append('<b> Objeto: </b>' + retorno[i].objeto_obra))
                                .append($('<td width="5%">').append('<center><button class="btn btn-secondary btn-sm"      onclick="listarArquivos(' + retorno[i].id + ', \'obras\',\'tbody_arquivos_modal\')"       data-toggle="modal" data-target="#modal-files"><i class="fa fa-file-text"></i></button></center>'))
...
function  update(id) {
    $.ajax({
        url: '../../_php/Dispatch.php?controller=ControllerObras&&action=update&&id=' + id,
        type: 'POST',
...
function insert() {
    $.ajax({
        url: '../../_php/Dispatch.php?controller=ControllerObras&&action=insert',
        type: 'POST',
        data: $('#tab-1-form-1').serialize(),
        beforeSend: function () {
        ...
        success: function (data) {
            if (!isNaN(data)) {
                ...
                document.getElementById("tab-3-btn-show-modal-upload").addEventListener('click', () => {
                    showModalUpload('modal-upload', 'ControllerUploadMultiplo', 'analisaArquivo', data , 'obras')  
                });
...
function deletar(id) {
    if (confirm("TEM CERTEZA QUE DESEJA EXCLUIR ESSE REGISTRO?")) {
        $.ajax({
            url: '../../_php/Dispatch.php?controller=ControllerObras&&action=delete',
            type: "POST",
            data: {id: id},
            success: function (data) {
                $("#linha" + id).remove();
