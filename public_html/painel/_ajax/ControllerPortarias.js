...
function populaForm(id) {
    $.ajax({
        url: '../../_php/Dispatch.php?controller=ControllerPortarias&&action=locate',
        type: 'POST',
        dataType: "json",
        data: {id: id},
        beforeSend: function () {
            ...
            if(retorno.length === 0){
                ...
                function paginar() {
                    ...
                    $("#tbody").append(
                            ...
                            .append($('<td>').append('<b>Exerc.: </b>' + retorno[i].exercicio + '<br/> <b>Data: </b>' + retorno[i].data_publicacao + '<br/> <b>Num.: </b>' + retorno[i].numero+'<br/><b>Tipo: </b>' + retorno[i].tipo+ '<br/><b>Nome: </b>' + retorno[i].nome))
                            .append($('<td width="5%">').append('<center><button class="btn btn-secondary btn-sm"      onclick="listarArquivos(' + retorno[i].id + ', \'portarias\',\'tbody_arquivos_modal\')"       data-toggle="modal" data-target="#modal-files"><i class="fa fa-file-text"></i></button></center>'))
...
function  update(id) {
    $.ajax({
        url: '../../_php/Dispatch.php?controller=ControllerPortarias&&action=update&&id=' + id,
        type: 'POST',
...
function insert() {
    $.ajax({
        url: '../../_php/Dispatch.php?controller=ControllerPortarias&&action=insert',
        type: 'POST',
        data: $('#tab-1-form-1').serialize(),
        beforeSend: function () {
        ...
        success: function (data) {
            if (!isNaN(data)) {
                ...
                document.getElementById("tab-2-btn-show-modal-upload").addEventListener('click', () =>{
                    showModalUpload('modal-upload', 'ControllerUploadMultiplo', 'analisaArquivo', data , 'portarias')  
                });
...
function deletar(id) {
    if (confirm("TEM CERTEZA QUE DESEJA EXCLUIR ESSE REGISTRO?")) {
        $.ajax({
            url: '../../_php/Dispatch.php?controller=ControllerPortarias&&action=delete',
            type: "POST",
            data: {id: id},
            success: function (data) {
                $("#linha" + id).remove();
...
function importarJson() {
    ...
    $.ajax({
      url: '../../_php/Dispatch.php?controller=ControllerPortarias&&action=importarJson',
      type: 'POST',
      data: formData,
      contentType: false,
      processData: false,
