function insertTab3Form1() {
    $.ajax({
        url: '../../_php/Dispatch.php?controller=ControllerLeisAlteracoes&&action=insert',
        type: 'POST',
        ...
        success: function (data) {
            if (!isNaN(data)) {
                ...
                locateTab3Form1($('#id_lei_alteradora').val());

                bootbox.confirm({
                    title: "Sucesso",
                    message: "Cadastro realizado com sucesso! Deseja enviar um arquivo agora?",
                    buttons: {
                        cancel: {
                            label: "Não",
...
function locateTab3Form1(id) {
    ...
    $.ajax({
        url: '../../_php/Dispatch.php?controller=ControllerLeisAlteracoes&&action=locate',
        cache: false,
        type: 'POST',
        data: {id: id},
        dataType: "json",
        ...
        success: function (retorno) {
            if (!retorno[0]) {
                $("#tab-3-echos-1").html("<h6 class='text-muted text-center'>Não há dados cadastraos!</h6>");
            } else {
                ...
                for (var i = pagina * tamanhoPagina; i < retorno.length && i < (pagina + 1) * tamanhoPagina; i++) {
                    $("#tab-3-tbody-1").append(
                            ...
                            .append($('<td width="5%">').append('<center><button class="btn btn-secondary btn-sm" onclick="listarArquivos(' + retorno[i].id_lei_alterada + ', \'leis\',\'tbody_arquivos_modal\')" data-toggle="modal" data-target="#modal-files"><i class="fa fa-file-text"></i></button></center>'))
                            .append($('<td style="width: 5%">').append('<center><button class="btn btn-danger btn-sm" onclick="deletarTab3Form1(' + retorno[i].id + ')"><i class="fa fa-remove"></i></button></center>'))
                            );
                }
            }
        }
...
function populaFormTab3Form1(id) {
    $.ajax({
        url: '../../_php/Dispatch.php?controller=ControllerLeisAlteracoes&&action=locate&&id=' + id,
        type: 'POST',
        dataType: "json",
        data: {id: id},
...
function  updateTab3Form1(id) {
    $.ajax({
        url: '../../_php/Dispatch.php?controller=ControllerLeisAlteracoes&&action=update&&id=' + id,
        type: 'POST',
        data: $('#tab-3-form-1').serialize(),
        beforeSend: function () {
...
function deletarTab3Form1(id) {
    if (confirm("\u{26A0}\u{FE0F} TEM CERTEZA QUE DESEJA EXCLUIR ESSE REGISTRO?")) {
        $.ajax({
            url: '../../_php/Dispatch.php?controller=ControllerLeisAlteracoes&&action=delete',
            type: "POST",
            data: {id: id},
            success: function (data) {
                $("#linha" + id).remove();
