...
function populaForm(id) {
    $.ajax({
        url: '../../_php/Dispatch.php?controller=ControllerCanaisAtendimento&&action=locate',
        type: 'POST',
        dataType: "json",
        data: {id: id},
...
function locate() {
    ...
    $('thead').append(
            ...
            .append($('<th>').append('Órgão'))
            .append($('<th>').append('Atendimento'))
            .append($('<th>').append('Canais'))
            .append($('<th>').append('tipo de Demanda'))
            .append($('<th>').append(''))
            .append($('<th>').append(''))
            );
...
//                    var tbody = $('table > tbody');
                    ...
                    for (var i = pagina * tamanhoPagina; i < retorno.length && i < (pagina + 1) * tamanhoPagina; i++) {
                        ...
                        $("#tbody").append(
                                ...
                                .append($('<td>').append(retorno[i].orgao))
                                .append($('<td>').append(retorno[i].atendimento))
                                .append($('<td>').append(retorno[i].canais))
                                .append($('<td>').append(retorno[i].tipo_demanda))                        
...
function  update(id) {
    $.ajax({
        url: '../../_php/Dispatch.php?controller=ControllerCanaisAtendimento&&action=update&&id=' + id,
        type: 'POST',
...
function insert() {
    $.ajax({
        url: '../../_php/Dispatch.php?controller=ControllerCanaisAtendimento&&action=insert',
        type: 'POST',
        data: $('#form_insert').serialize(),
        beforeSend: function () {
...
function deletar(id) {
    if (confirm("TEM CERTEZA QUE DESEJA EXCLUIR ESSE REGISTRO?")) {
        $.ajax({
            url: '../../_php/Dispatch.php?controller=ControllerCanaisAtendimento&&action=delete',
            type: "POST",
            data: {id: id},
            success: function (data) {
                $("#linha" + id).remove();
