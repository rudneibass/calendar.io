...
function populaForm(id) {
    $.ajax({
        url: '../../_php/Dispatch.php?controller=ControllerResponsabilidadeFiscal&&action=locate',
        type: 'POST',
        dataType: "json",
        data: {id: id},
        beforeSend: function () {
...
function locate() {
    ...
    $('#thead').append(
            ...
            .append($('<th>').append('Tipo'))
            .append($('<th>').append('Período'))
            .append($('<th>').append('Exercicio'))
            .append($('<th>'))
            .append($('<th>'))
            .append($('<th>'))
            );
...
//                    var tbody = $('table > tbody');
                    ...
                    for (var i = pagina * tamanhoPagina; i < retorno.length && i < (pagina + 1) * tamanhoPagina; i++) {
                        ...
                        $("#tbody").append(
                                ...
                                .append($('<td>').append(retorno[i].tipo_lrf))
                                .append($('<td>').append(retorno[i].periodo))
                                .append($('<td>').append(retorno[i].exercicio))
                                .append($('<td width="5%">').append('<center><button class="btn btn-secondary btn-sm"      onclick="listarArquivos(' + retorno[i].id + ', \'responsabilidade\',\'tbody_arquivos_modal\')"       data-toggle="modal" data-target="#modal-files"><i class="fa fa-file-text"></i></button></center>'))
...
function  update(id) {
    $.ajax({
        url: '../../_php/Dispatch.php?controller=ControllerResponsabilidadeFiscal&&action=update&&id=' + id,
        type: 'POST',
...
function insert() {
    $.ajax({
        url: '../../_php/Dispatch.php?controller=ControllerResponsabilidadeFiscal&&action=insert',
        type: 'POST',
        data: $('#tab-1-form-1').serialize(),
        beforeSend: function () {
        ...
        success: function (data) {
            if (!isNaN(data)) {
				...
				$("#tab-2-btn-show-modal-upload").attr("onclick", "showModalUpload('modal-upload', 'ControllerUploadMultiplo', 'analisaArquivo', " + data + ", 'responsabilidade')");
                ...
                document.getElementById("tab-2-btn-show-modal-upload").addEventListener('click', () =>{
                    showModalUpload('modal-upload', 'ControllerUploadMultiplo', 'analisaArquivo', data , 'responsabilidade')  
                });
...
function deletar(id) {
    if (confirm("TEM CERTEZA QUE DESEJA EXCLUIR ESSE REGISTRO?")) {
        $.ajax({
            url: '../../_php/Dispatch.php?controller=ControllerResponsabilidadeFiscal&&action=delete',
            type: "POST",
            data: {id: id},
            success: function (data) {
                $("#linha" + id).remove();
