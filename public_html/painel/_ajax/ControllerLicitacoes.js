...
function populaForm(id) {
    $.ajax({
        url: '../../_php/Dispatch.php?controller=ControllerLicitacoes&&action=locate',
        type: 'POST',
        dataType: "json",
        data: {id: id},
        beforeSend: function () {
...
//                    var tbody = $('table > tbody');
                    ...
                    for (var i = pagina * tamanhoPagina; i < retorno.length && i < (pagina + 1) * tamanhoPagina; i++) {
                        ...
                        $("#tbody").append(
                                ...
                                .append($('<td style="padding: 15px 10px; width: 15%">').append('<b><span class="span-14">' + retorno[i].processo + '</b><br/>' + retorno[i].modalidade))
                                .append($('<td style="padding: 15px 10px">').append('<b><span class="span-14">Publicação: </b>' + retorno[i].data_publicacao_edital + ' &emsp; <b>Tipo:</b>' + retorno[i].tipo + '</b><br/><p><span class="span-14">' + retorno[i].objeto_licitacao))
                                .append($('<td style="padding: 15px 10px">').append('<b><span class="span-14">' + retorno[i].data_abertura + '</b><br/>' + retorno[i].situacao))
                                .append($('<td style="width: 5%">').append('<center><button class="btn btn-secondary btn-sm" onclick="listarArquivos(' + retorno[i].id + ', \'licitacoes\',\'tbody_arquivos_modal\')" data-toggle="modal" data-target="#modal-files"><i class="fa fa-file-text"></i></button></center>'))
...
function  update(id) {
    $.ajax({
        url: '../../_php/Dispatch.php?controller=ControllerLicitacoes&&action=update&&id=' + id,
        type: 'POST',
...
function insert() {
    $.ajax({
        url: '../../_php/Dispatch.php?controller=ControllerLicitacoes&&action=insert',
        type: 'POST',
        data: $('#tab-1-form-1').serialize(),
        beforeSend: function () {
        ...
        success: function (data) {
            if (!isNaN(data)) {
                ...
                document.getElementById("tab-4-btn-show-modal-upload").addEventListener('click', () => {
                    showModalUpload('modal-upload', 'ControllerUploadMultiplo', 'analisaArquivo', data , 'convenios')  
                });
...
function deletar(id) {
    if (confirm("TEM CERTEZA QUE DESEJA EXCLUIR ESSE REGISTRO?")) {
        $.ajax({
            url: '../../_php/Dispatch.php?controller=ControllerLicitacoes&&action=delete',
            type: "POST",
            data: {id: id},
            success: function (data) {
                $("#linha" + id).remove();
...
function importarJson() {
    ...
    $.ajax({
      url: '../../_php/Dispatch.php?controller=ControllerLicitacoes&&action=importarJson',
      type: 'POST',
      data: formData,
      contentType: false,
      processData: false,
