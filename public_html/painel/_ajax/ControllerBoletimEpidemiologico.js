...
function populaForm(id) {
    $.ajax({
        url: '../../_php/Dispatch.php?controller=ControllerBoletimEpidemiologico&&action=locate',
        type: 'POST',
        dataType: "json",
        data: {id: id},
        beforeSend: function () {
...
//                    var tbody = $('table > tbody');
                    ...
                    for (var i = pagina * tamanhoPagina; i < retorno.length && i < (pagina + 1) * tamanhoPagina; i++) {
                        ...
                        $("#tbody").append(
                                ...
                                .append($('<td>').append(retorno[i].hora))
                                .append($('<td>').append(retorno[i].suspeitos))
                                .append($('<td>').append(retorno[i].confirmados))
                                .append($('<td>').append(retorno[i].descartados))
                                .append($('<td>').append(retorno[i].internados))
                                .append($('<td>').append(retorno[i].obitos))
                                .append($('<td>').append(retorno[i].curados))
                                .append($('<td>').append(retorno[i].notificados))
...
function  update(id) {
    $.ajax({
        url: '../../_php/Dispatch.php?controller=ControllerBoletimEpidemiologico&&action=update&&id=' + id,
        type: 'POST',
...
function insert() {
    $.ajax({
        url: '../../_php/Dispatch.php?controller=ControllerBoletimEpidemiologico&&action=insert',
        type: 'POST',
        data: $('#tab-1-form-1').serialize(),
        beforeSend: function () {
...
function deletar(id) {
    if (confirm("TEM CERTEZA QUE DESEJA EXCLUIR ESSE REGISTRO?")) {
        $.ajax({
            url: '../../_php/Dispatch.php?controller=ControllerBoletimEpidemiologico&&action=delete',
            type: "POST",
            data: {id: id},
            success: function (data) {
                $("#linha" + id).remove();
