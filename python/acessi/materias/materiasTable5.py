...
def extract_first_table_data(url):
    ...
    if response.status_code == 200:
        ...
        if len(table) >= 5:
            if table[4]:
                headers = [unidecode(header.text.strip().lower()) for header in table[4].find_all('th')]

                for row in table[4].find_all('tr')[1:]:
...
def extract_and_save_data(url, all_data):
    ...
    if table_data:
        ...
        print(f"Os dados da página {url} foram extraídos e salvos em 'materias_table_5.json'.")
    else:
...
for page_number in range(2000, total_pages + 1):
    current_url = f"{base_url}{page_number}"
    extract_and_save_data(current_url, all_data)

json_data = json.dumps(all_data, indent=2, ensure_ascii=False)

with open('materias_table_5.json', 'w', encoding='utf-8') as json_file:
    json_file.write(json_data)

print("Todos os dados foram extraídos e salvos em 'materias_table_5.json'.")
