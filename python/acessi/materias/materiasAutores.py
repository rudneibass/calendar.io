...
def extract_first_table_data(url):
    ...
    if response.status_code == 200:
        ...
        if not autores:
            print(f"Não há atores na página {url}. ")
            return
...
def extract_and_save_data(url, all_data):
    ...
    if table_data:
        ...
        print(f"Os dados da página {url} foram extraídos e salvos em 'autores.json'.")
    else:
...
print("Todos os dados foram extraídos e salvos em 'autores.json'.")
