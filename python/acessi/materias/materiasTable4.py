...
def extract_first_table_data(url):
    ...
    if response.status_code == 200:
        ...
        if len(table) >= 4:
            if table[3]:
                headers = [unidecode(header.text.strip().lower()) for header in table[3].find_all('th')]

                for row in table[3].find_all('tr')[1:]:
...
def extract_and_save_data(url, all_data):
    ...
    if table_data:
        ...
        print(f"Os dados da página {url} foram extraídos e salvos em 'materias_table_4.json'.")
    else:
...
with open('materias_table_4.json', 'w', encoding='utf-8') as json_file:
    json_file.write(json_data)

print("Todos os dados foram extraídos e salvos em 'materias_table_4.json'.")
