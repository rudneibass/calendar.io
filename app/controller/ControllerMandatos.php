...
class ControllerMandatos {
    ...
    function locate() {
        ...
        if (isset($_POST['id'])) {
            ...
            $prep = "WHERE m.id=" . filter_input(INPUT_POST, 'id', FILTER_SANITIZE_NUMBER_INT) . " ";
            $crud = new ClassModel();
            $select = $crud->select('*, m.id as relacionamento  ', 'mandatos m', $prep, array());
            ...
            $crud = new ClassModel();
            $select = $crud->select("*, DATE_FORMAT(data_cadastro, '%d/%m/%Y') as data_cadastro", 'mandatos ', $prep, array());
    ...
    public function update() {
        ...
        if ($val->validar()) {
            ...
            $crud = new ClassModel();
            $update = $crud->update('mandatos ', $post, $id);
        } else {
    ...
    public function insert() {
        ...
        if ($val->validar()) {
            ...
            $crud = new ClassModel();
            print $crud->insert($post, 'mandatos ');
        } else {
            foreach ($val->getErrors() as $erro) {
                echo "$erro <br />";
            }
    ...
    /*public function delete() {
        ...
        if ($val->validar()) {
            ...
            $crud = new ClassModel();
            $delete = $crud->delete('mandatos ', ' WHERE id=?', array($id));
        } else {
            foreach ($val->getErrors() as $erro) {
                echo "$erro <br />";
            }
