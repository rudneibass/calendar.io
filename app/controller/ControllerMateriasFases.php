...
class ControllerMateriasFases {
    ...
    function locate() {
        ...
        if (isset($_GET['id'])) {
            ...
            $crud = new ClassModel();
            $select = $crud->select('* ', 'materias_fases ', $prep, array());
            $result = $select->fetch(\PDO::FETCH_ASSOC); // A T E N Ç Ã O !!! AQUI O PDO MONTA UM ARRAY ASSOCIATIVO.
            echo json_encode($result, JSON_PRETTY_PRINT);
        } else {

            $prep = "WHERE id_materias = " . $_POST['id'] . "";
            $crud = new ClassModel();
            $select = $crud->select("*, DATE_FORMAT(data_fase, '%d/%m/%Y') as data_fase ", "materias_fases ", $prep, array());
    ...
    public function update() {
        ...
        if ($val->validar()) {
            ...
            $crud = new ClassModel();
            $crud->update('materias_fases ', $post, $id);
        } else {
    ...
    public function insert() {
        ...
        if ($val->validar()) {
            ...
            $post['usuario'] = $_SESSION['USUARIO'];
            $crud = new ClassModel();
            print $crud->insert($post, 'materias_fases ');
        } else {
            foreach ($val->getErrors() as $erro) {
                echo "$erro <br />";
            }
    ...
    public function delete() {
        ...
        if ($val->validar()) {
            ...
            $crud = new ClassModel();
            $delete = $crud->delete('materias_fases ', ' WHERE id=?', array($id));
        } else {
            foreach ($val->getErrors() as $erro) {
                echo "$erro <br />";
            }
