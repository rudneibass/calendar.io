...
class ControllerCanaisAtendimento {
    ...
    function locate() {
        ...
        if (isset($_POST['id'])) {
            ...
            $crud = new ClassModel();
            $select = $crud->select('* ', 'canais_atendimento ', $prep, array());
            ...
            $crud = new ClassModel();
            $select = $crud->select("*, DATE_FORMAT(data_cadastro, '%d/%m/%Y') as data_cadastro", 'canais_atendimento ', $prep, array());
    ...
    public function update() {
        ...
        $val->set($_POST['orgao'], '<b>"Órgão"</b>')->obrigatorio();
        ...
        if ($val->validar()) {
            ...
            $crud = new ClassModel();
            $update = $crud->update('canais_atendimento ', $post, $id);
        } else {
    ...
    public function insert() {
        ...
        if ($val->validar()) {
            ...
            $crud = new ClassModel();
            print $crud->insert($post, 'canais_atendimento ');
        } else {
            foreach ($val->getErrors() as $erro) {
                echo "$erro <br />";
            }
    ...
    /*public function delete() {
        ...
        if ($val->validar()) {
            ...
            $crud = new ClassModel();
            $delete = $crud->delete('canais_atendimento ', ' WHERE id=?', array($id));
        } else {
            foreach ($val->getErrors() as $erro) {
                echo "$erro <br />";
            }
