...
class ControllerServicos {
    ...
    function teste() {
        echo 'function teste() Controller Serviços Controller';
    }
    ...
    function locate() {
        ...
        if (isset($_POST['id'])) {
            ...
            $crud = new ClassModel();
            $select = $crud->select('*, e.id as relacionamento  ', 'servicos e', $prep, array());
            ...
            $crud = new ClassModel();
            $select = $crud->select("*, DATE_FORMAT(data_cadastro, '%d/%m/%Y') as data_cadastro", 'servicos ', $prep, array());
    ...
    public function update() {
        ...
        $val->set($_POST['nome'], 'Titulo')->obrigatorio();
        ...
        if ($val->validar()) {
            ...
            $crud = new ClassModel();
            $update = $crud->update('servicos ', $post, $id);
        } else {
    ...
    public function insert() {
        ...
        if ($val->validar()) {
            ...
            $post['usuario'] = $_SESSION['USUARIO'];
            $crud = new ClassModel();
            print $crud->insert($post, 'servicos ');
        } else {
            foreach ($val->getErrors() as $erro) {
                echo "$erro <br />";
            }
    ...
    public function delete() {
        ...
        if ($val->validar()) {
            ...
            $crud = new ClassModel();
            $delete = $crud->delete('servicos ', ' WHERE id=?', array($id));
        } else {
            foreach ($val->getErrors() as $erro) {
                echo "$erro <br />";
            }
