...
class ControllerEntidadeOrgaosGestor {
    ...
    public function update() {
        ...
        if (isset($post['ativo'])) {
            $select = $crud->select('* ', 'entidade_orgaos_membros ', 'WHERE id_cargo = '.$_POST['id_cargo'].' and ativo = "on" ', array());
            $result = $select->fetchAll();
            if($result){
                foreach($result as $data){
                    $crud->update('entidade_orgaos_membros ', ['ativo' => 'off'], $data['id']);
                }
            }
        }
        ...
        if ($val->validar()) {
            ...
            $crud->update('entidade_orgaos_membros ', $post, $id);
        } else {
    ...
    public function insert() {
        ...
        if ($val->validar()) {
            print $crud->insert($post, 'entidade_orgaos_membros ');
        } else {
            foreach ($val->getErrors() as $erro) {
                echo "$erro <br />";
            }
    ...
    /*public function delete() {
        ...
        if ($val->validar()) {
            ...
            $crud = new ClassModel();
            $delete = $crud->delete('entidade_orgaos_membros ', ' WHERE id=?', array($id));
        } else {
            foreach ($val->getErrors() as $erro) {
                echo "$erro <br />";
            }
